...
exports.retrieved_answers = (req, res, next) => {
    const connection = DB_connect()
    connection.query('SELECT * FROM answers ORDER BY id ASC ', (error, all_answers) => {
        if (error) {
            console.error('❌SQL error❌: ', error);
            return next(error);
        }
...
exports.create_article = (req, res, next) => {
    ...
    const connection = DB_connect()
    connection.query(`INSERT INTO articles(creatorId, message, pseudo, image, genre, dates, voteFor, voteAgainst, userFor, userAgainst,valide) VALUES ("${article.creator}", "${article.message}", "${article.pseudo}", "${article.image}", "${article.genre}"," ${article.dates}"," ${article.voteFor}"," ${article.voteAgainst}"," ${article.userFor}", "${article.userAgainst}","${0}")
    `,(error,result)=>{
        if (error) {
            return res.status(505).json({error: error})
        }
...
exports.retrieved_all = (req, res, next) => {
    const connection = DB_connect()
    connection.query('SELECT * FROM articles ORDER BY dates DESC ', (error, all_articles) => {
        if (error) {
            console.error('❌SQL error❌: ', error);
            return next(error);
        }
